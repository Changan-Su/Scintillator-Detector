...
def plot_scatter_views(data_df: pd.DataFrame, output_dir: Path) -> None:
    """
    For each algorithm, produce a 1×3 figure with three orthogonal scatter views:
      - XY plane  (looking along Z)
      - XZ plane  (looking along Y)
      - YZ plane  (looking along X)
    Reconstructed positions: blue dots.
    True positions: red crosses.
    Origin (0,0) marked on each subplot.
    """
    for algo, group in data_df.groupby("algorithm", sort=False):
        ...
        # All positions are in cm: true positions converted on read, rec positions native cm
        views = [
            ("XY plane (view along Z)", "x", "y", "rec_x", "rec_y", "true_x", "true_y"),
            ("XZ plane (view along Y)", "x", "z", "rec_x", "rec_z", "true_x", "true_z"),
            ("YZ plane (view along X)", "y", "z", "rec_y", "rec_z", "true_y", "true_z"),
        ]

        for ax, (title, hlabel, vlabel, rec_h, rec_v, true_h, true_v) in zip(axes, views):
            ax.axhline(0, color="gray", linewidth=0.6, linestyle="--")
            ax.axvline(0, color="gray", linewidth=0.6, linestyle="--")

            ax.scatter(
                group[rec_h], group[rec_v],
                s=18, alpha=0.7, color="steelblue",
                label="Reconstructed", zorder=3,
            )
            ax.scatter(
                group[true_h], group[true_v],
                s=40, marker="+", linewidths=1.4, color="crimson",
                label="True", zorder=4,
            )

            ax.set_xlabel(f"{hlabel.upper()} (cm)", fontsize=10)
            ax.set_ylabel(f"{vlabel.upper()} (cm)", fontsize=10)
